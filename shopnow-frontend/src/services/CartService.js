...
class CartService {
  ...
  getByName(name) {
    return axios.get(`shopping-cart/by-name/${name}`, {
      headers: AuthHeader(),
    });
  }

  create(name) {
    return axios.post(`shopping-cart?name=${name}`, null, {
      headers: AuthHeader(),
    });
  }
  ...
  deleteProduct(cartId, productId) {
    return axios.delete(`shopping-cart/${cartId}/products/${productId}`, {
      headers: AuthHeader(),
    });
  }

  getTotalPrice(cartId) {
    return axios.get(`shopping-cart/totalprice/${cartId}`, {
      headers: AuthHeader(),
    });
  }
  ...
  addProducts(cartId, data) {
    return axios.post(`shopping-cart/${cartId}`, data, {
      headers: AuthHeader(),
    });
  }
}
